...
print(mail_df = mail_df.fillna(value=0)) # обработка отсутствующих данных (замена NaN на 0)
print(uname_df = uname_df.fillna(value=0))
...
# b) Создание визуализации распределения набора данных. Создать произвольный датасет
# через np.random.normal или использовать датасет из csv-файлов, потом построить гистограмму.
...
print(f"Среднее значение по dats.csv: {mean_sales_data}")
print(f"Стандартное отклонение по dats.csv: {std_sales_data}")
print(f"Максимальное значение по dats.csv: {max_sales_data}")
...
print(f"\nСреднее значение по sales.csv: {mean_price}")
print(f"Стандартное отклонение по sales.csv: {std_price}")
print(f"Максимальное значение по sales.csv: {max_price}")


# Задание 7.
# Операции с матрицами: Используя numpy, напишите сценарий, который создает матрицу и выполняет
# основные математические операции, такие как сложение, вычитание, умножение и транспонирование матрицы.
